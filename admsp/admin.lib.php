...
{
    global $is_admin, $is_supplier;

    if ($is_admin == 'super' || $is_supplier) return;
...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_SP_ADMIN_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
function admin_referer_check($return=false)
{
    ...
    if( $p['path'] && ! preg_match( '/\/'.preg_quote(G5_SP_ADMIN_DIR).'\//i', $p['path'] ) ){
        $msg = '올바른 방법으로 이용해 주십시오';
    }
...
if (!$member['mb_id'])
{
    alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_SP_ADMIN_URL));
}
else if ($is_admin != 'super' && ! $is_supplier )
{
    $auth = array();
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_SP_ADMIN_PATH.'/'.$entry;
}
