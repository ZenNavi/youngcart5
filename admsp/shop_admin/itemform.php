        ...
        <tr>
            ...
            <td colspan="2">
                ...
                <?php } else { ?>
                    ...
                    <a href="<?php echo G5_SP_ADMIN_URL; ?>/shop_admin/itemuselist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">사용후기</a>
                    <a href="<?php echo G5_SP_ADMIN_URL; ?>/shop_admin/itemqalist.php?sfl=a.it_id&amp;stx=<?php echo $it_id; ?>" class="btn_frmline">상품문의</a>
                <?php } ?>
            </td>
        </tr>
        <tr>
...
<section id="anc_sitfrm_compact">
    ...
    <div id="sit_compact_fields"><?php include_once(G5_SP_ADMIN_PATH.'/shop_admin/iteminfo.php'); ?></div>
</section>
...
$(function(){
    $(document).on("change", "#it_info_gubun", function() {
        var gubun = $(this).val();
        $.post(
            "<?php echo G5_SP_ADMIN_URL; ?>/shop_admin/iteminfo.php",
...
<section id="anc_sitfrm_cost">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                <input type="number" name="it_price" value="<?php echo $it['it_price']; ?>" id="it_price" class="frm_input currency" size="8"> 원
            </td>
            <td class="td_grpset">
        ...
        <tr>
            ...
            <td>
                <?php echo help("입력하지 않으면 상품상세페이지에 출력하지 않습니다."); ?>
                <input type="number" name="it_cust_price" value="<?php echo $it['it_cust_price']; ?>" id="it_cust_price" class="frm_input currency" size="8"> 원
            </td>
            <td class="td_grpset">
        ...
        <tr>
            <th scope="row"><label for="it_supp_price">공급가격</label></th>
            <td>
                <?php echo help("입력하지 않으면 공급사에 정산에서 제외 됩니다."); ?>
                <input type="number" name="it_supp_price" id="it_supp_price" value="<?php echo $it['it_supp_price']; ?>" size="8" class="frm_input currency"> 원
            </td>
            <td class="td_grpset">
                <input type="checkbox" name="chk_ca_it_supp_price" value="1" id="chk_ca_it_supp_price">
                <label for="chk_ca_it_supp_price">분류적용</label>
                <input type="checkbox" name="chk_all_it_supp_price" value="1" id="chk_all_it_supp_price">
                <label for="chk_all_it_supp_price">전체적용</label>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="2">
                <div class="sit_option tbl_frm01">
                    ...
                    <style>
                        ul.controller--io-options { display: table;}
                        ul.controller--io-options li{ display: table-cell; }
                        ul.controller--io-options:after{content: ' '; display: table; clear : both;}
                        .holder--io-options>tr:first-of-type .controller--io-options li button { visibility: hidden;}
                        .holder--io-options>tr:first-of-type .controller--io-options li:last-of-type button { visibility: visible;}
                        .holder--io-options label { font-weight: bold; }
                    </style>
                    <table>
                    ...
                    <tbody class="holder--io-options" data-min-row="1" data-max-row="10">
                    <?php foreach($opt_subject as $i=>$tmp_subject) : ?>
                    <tr data-for-option="<?php echo ($i+1); ?>">
                        <th scope="row">
                            <label for="opt<?php echo ($i+1); ?>_subject">옵션<?php echo ($i+1); ?></label>
                            <input type="text" name="opt<?php echo ($i+1); ?>_subject" value="<?php echo $opt_subject[$i]; ?>" id="opt<?php echo ($i+1); ?>_subject" class="frm_input" size="15">
                        </th>
                        <td>
                            <label for="opt<?php echo ($i+1); ?>">옵션<?php echo ($i+1); ?> 항목</label>
                            <input type="text" name="opt<?php echo ($i+1); ?>" value="" id="opt<?php echo ($i+1); ?>" class="frm_input" size="50">
                        </td>
                        <td>
                            <ul class="controller--io-options">
                                <li><button type="button" class="btn btn_01"><i class="fa fa-plus"></i></button></li>
                                <li><button type="button" class="btn btn_01"><i class="fa fa-minus"></i></button></li>
                                <li><button type="button" class="btn btn_01"><i class="fa fa-plus"></i></button></li>
                            </ul>
                        </td>
                    </tr>
                ...
                <div id="sit_option_frm"><?php include_once(G5_SP_ADMIN_PATH.'/shop_admin/itemoption.php'); ?></div>
                <script>
                    (function($){
                        $(document).on('ready', function(){
                            var $holderIoOptions = $('tbody.holder--io-options');

                            var initialOptNumber = function(){
                                var $trs = $holderIoOptions.find('>tr');
                                for( var i = 0, imax = $trs.size(); i < imax ; i++ ){
                                    var $tr = $($trs.get(i));
                                    $tr.attr('data-for-option', (i+1));
                                    $tr.find('label').each(function(){
                                        $(this).text( $(this).text().replace(/옵션(\d+)/g, '옵션'+(i+1)) );
                                        $(this).attr('for', $(this).attr('for').replace(/opt(\d+)/g, 'opt'+(i+1)));
                                    });
                                    $tr.find('input').each(function(){
                                        $(this).attr('name', $(this).attr('name').replace(/opt(\d+)/g, 'opt'+(i+1)));
                                        $(this).attr('id', $(this).attr('id').replace(/opt(\d+)/g, 'opt'+(i+1)));
                                    });
                                }
                            };

                            $('.controller--io-options>li>button').on('click', function(){
                                var actType = $(this).closest('li').index();
                                var $curTr  = $(this).closest('tr');

                                var curCnt  = $holderIoOptions.find('>tr').size();
                                var maxRow  = $holderIoOptions.data('max-row');
                                var minRow  = $holderIoOptions.data('min-row');


                                if( actType == 1 ) {
                                    if( curCnt == minRow ) {
                                        alert(minRow + ' 이상이여야 합니다.');
                                        return;
                                    }
                                    $(this).closest('tr').remove();
                                } else {
                                    if( curCnt == maxRow ) {
                                        alert(maxRow + '이하여야 힙니다.');
                                        return;
                                    }
                                    var $tr     = $(this).closest('tbody').find('tr:first-of-type').clone(true);
                                    $tr.find('input').val('');
                            ...
                            $("#option_table_create").click(function() {
                                ...
                                var param = { it_id: it_id, w: "<?php echo $w; ?>"
                                ...
                                $.post(
                                    "<?php echo G5_SP_ADMIN_URL; ?>/shop_admin/itemoption.php",
                                    param,
                                    function(data) {
                            ...
                            $(document).on("click", "#opt_value_apply", function() {
                                ...
                                var opt_supp_price = $.trim($("#opt_com_supp_price").val());
                                ...
                                if($el.size() > 0) {
                                    ...
                                    $el.each(function() {
                                        ...
                                        if($("#opt_com_supp_price_chk").is(":checked"))
                                            $tr.find("input[name='opt_supp_price[]']").val(opt_supp_price);
                                    ...
                                    if($("#opt_com_supp_price_chk").is(":checked"))
                                        $("input[name='opt_supp_price[]']").val(opt_supp_price);
        ...
        <tr>
            ...
            <td colspan="2">
                ...
                <div id="sit_option_addfrm"><?php include_once(G5_SP_ADMIN_PATH.'/shop_admin/itemsupply.php'); ?></div>
                ...
                $(function() {
                    ...
                    $("#supply_table_create").click(function() {
                        ...
                        $.post(
                            "<?php echo G5_SP_ADMIN_URL; ?>/shop_admin/itemsupply.php",
