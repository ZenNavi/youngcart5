...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_BR_ADMIN_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
if (!$member['mb_id'])
{
    alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_BR_ADMIN_URL));
}
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_BR_ADMIN_PATH.'/'.$entry;
}
@asort($menu_files);
